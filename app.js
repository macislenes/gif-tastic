...
// On document.ready you are going to create an array of strings called animals and it's going load the following animals
$(document).ready(function() {
  ...
  function populateButtons(arrayToUse, classToAdd, areaToAddTo) {
    // here we are going to empty the areaToAddTo Div so we don't just keep adding buttons to the same space with the existing buttons
    $(areaToAddTo).empty();

    // using a for loop we are going to loop over our array of animals
    for (var i = 0; i < arrayToUse.length; i++) {
      // declaring a new variable and dynaically creating a new html button
      var a = $("<button>");
      // using .addClass() to assign the class that is passed to the function
      a.addClass(classToAdd);
      // we are adding a .attr() assigning a "data-type" for the button and giving it the value of the animal in the array
      // this will later be used by the ajax call to form the query to the giphy API
...
// here is our .on("click", function(){}) inside this we are listening for a click on the buttons with the ".animal-button" class that we dynamically created 
  $(document).on("click", ".animal-button", function() {
    ...
    // that way anything that was previously highlighted as "active" will no longer be
    ...
    // that way we can later use that type to search within giphy for that specific elemet name
    var type = $(this).attr("data-type");
    // declaring a new variable and assining it the giphy url we want to hit with an ajax call
    // building the url by feeding 3 parameters q, api_key and limit
    // q, is the type (the "data-type") of the button which was assigned the animal name
    // the api_key which is our api key 
    // limit which we have set to 10 ==> telling the api to return 10 reults at most
    ...
    // we will passing the (response) from the api through our funcion 
      .then(function(response) {
        ...
        for (var i = 0; i < results.length; i++) {
          ...
          // declaring a new variable called rating and assigning it the value of the .rating return from within theresults[i] as it loop 
          var rating = results[i].rating;
          // dynamically creating a new <p> tag and assigning the text as the rating of the gif
          var p = $("<p>").text("Rating: " + rating);

          // assigning a new animated variable as the .images.fixed.heigh.url from within the results returned from the api
          var animated = results[i].images.fixed_height.url;
          // assigning a new still variable as the .images.fixed.heigh_still.url from within the results
          var still = results[i].images.fixed_height_still.url;
          // dynamically creating a new <img> element 
          var animalImage = $("<img>");
          // assigning varirous data values we will need to show the still and moving images
...
// here is our .on("click", function(){}) inside this we are listening for a click on the images with the ".animal-image" class that we dynamically created . this function will pause a moving image or unpause a still image.
$(document).on("click", ".animal-image", function() {

  // declaring a new variable called state and assigning it the return of the "data-state".attr  of the item clicked using $(this) 
    var state = $(this).attr("data-state");
    // an if() statement saying if the return of the state is "still" run this code within the {}
    if (state === "still") {
      // assigning a new .attr to the item clicked using $(this)
      // we are assigning a new source as "data-animate" 
      // "data-animate" was assined when we dynamicalled created the elements
      // "data-animate" contains the url to the animated version of the gif
    ...
    // if the above statement is not true run this code
      ...
      // assigning a new .attr to the item clicked using $(this)
      // we are assigning a new source as "data-still" 
      // "data-still" was assined when we dynamicalled created the elements
      // "data-still" contains the url to the still version of the gif
...
// here is our .on("click", function(){}) inside this we are listening for a click on the elements with the ".add-animal" class
  $("#add-animal").on("click", function(event) {
    ...
    // here is the call to the populateButtons() when the page is first loaded
      // passing animals through the arrayToUse
      // "animal-button" as the classToAdd 
      // "#animal-buttons" id of the html element for areaToAddTo
  populateButtons(animals, "animal-button", "#animal-buttons");
